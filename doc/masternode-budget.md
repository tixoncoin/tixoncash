...
Tixoncash now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<tixoncash_address\> \<monthly_payment_tixoncash\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ TXCH, _12_ times over the course of a year totaling _24000_ TXCH.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<tixoncash_address\> \<monthly_payment_tixoncash\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` TXCH.
 ...
 * vote-many          - Vote on a Tixoncash initiative
 * vote-alias         - Vote on a Tixoncash initiative
 * vote               - Vote on a Tixoncash initiative/budget
